...
// 🔹 Derived color map (for passing to children)
const STATUS_COLORS: Record<keyof typeof STATUS_CONFIG, string> = Object.fromEntries(
...
export default function CalendarPage() {
  ...
  const filteredEvents = useMemo(() => {
    return SAMPLE_EVENTS.filter((event) => {
      const matchesStatus =
        selectedStatuses.size === 0 || selectedStatuses.has(event.status);
      ...
      <div className="flex-1 flex w-full overflow-auto">
        ...
        <main className="flex-1 flex flex-col">
                    ...
                    return (
                      <Button
                        ...
                        className={`${
                          isActive
                            ? "text-white"
                            : "text-[#2160AD] hover:bg-[#2160AD]/10!"
                        }`}
                      >
                        {mode}
                      </Button>
          ...
          <section className="flex-1 overflow-hidden bg-white">
            {activeTab === "day" && (
              <DayView date={currentDate} events={filteredEvents} statusColors={STATUS_COLORS} />
            )}
            {activeTab === "week" && (
              <WeekView date={currentDate} events={filteredEvents} statusColors={STATUS_COLORS} />
            )}
            {activeTab === "month" && (
              <MonthView date={currentDate} events={filteredEvents} statusColors={STATUS_COLORS} />
            )}
          </section>
        </main>
      </div>
