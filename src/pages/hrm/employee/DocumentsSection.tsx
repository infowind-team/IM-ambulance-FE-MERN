...
interface SectionProps {
  title: string;
  icon: React.ReactNode;
  path: `documents.${string}`; // RHF path
}

function DocumentSection({ title, icon, path }: SectionProps) {
  const [open, setOpen] = useState(false);

  const { watch, setValue } = useFormContext();

  const documents: Document[] = watch(path) || [];
  ...
  const addDocument = (data: DocumentFormData) => {
    const newDoc: Document = {
      id: Date.now().toString(),
      title: data.title,
      filename: data.filename,
    ...
    setValue(path, [...documents, newDoc]);
          ...
          <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent>
              ...
              <form onSubmit={handleSubmit(addDocument)} className='space-y-4'>
                ...
                <div className='space-y-2'>
                  <Label>Filename</Label>
                  <Input {...register('filename')} placeholder='document.pdf' />
                  {errors.filename && (
  ...
  return (
    <div className='space-y-6'>
      ...
      <DocumentSection
        title='Identification Documents'
        icon={<IdCard className='w-5 h-5' />}
        path='documents.idDocuments'
      />

      <DocumentSection
        title='Employment-Related Documents'
        icon={<Folder className='w-5 h-5' />}
        path='documents.employmentDocs'
      />
    </div>
  );
}
