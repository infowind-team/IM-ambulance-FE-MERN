...
//       `http://192.168.1.73:9000/vehicles/get-all/${page}/${limit}`,
//       {
//           params: {
//               page,
//               limit,
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  const { page, limit, search, status, sortBy = "1", sortOrder = "1" } = req.query;

  try {
    if (!page || !limit) {
      return res.status(400).json({ message: "Page and limit are required query params." });
    }
